...
function App() {
  ...
  const isPartial =
    selectedVideos.size > 0 && selectedVideos.size < videos.length;
  ...
  const fetchVideoSettings = async (videoUrl, videoId) => {
    if (videoSettings[videoId]?.visible) {
      setVideoSettings((prev) => ({
        ...prev,
        [videoId]: { ...prev[videoId], visible: false },
      }));
      return;
    }
    setVideoSettings((prev) => ({
      ...prev,
      [videoId]: { visible: true, loading: true, formats: [], subs: [] },
    }));
    ...
    try {
      const fmtRes = await fetch(`${API_BASE}/api/list-formats`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "ngrok-skip-browser-warning": "true",
    ...
    try {
      const subRes = await fetch(`${API_BASE}/api/list-subs`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "ngrok-skip-browser-warning": "true",
    ...
    const defaultFormat =
      formats.find((f) => f.id === "video_720") || formats[0];
    if (defaultFormat)
      setSelectedFormats((prev) => ({ ...prev, [videoId]: defaultFormat }));

    setVideoSettings((prev) => ({
      ...prev,
      [videoId]: { visible: true, loading: false, formats, subs },
    }));
  };
  ...
  const cancelDownload = async (videoId) => {
    ...
    if (task_id) {
      try {
        await fetch(`${API_BASE}/api/cancel-download/${task_id}`, {
          method: "POST",
        });
      } catch (err) {
  ...
  const downloadAndAwaitVideo = (videoUrl, videoId) => {
    return new Promise((resolve, reject) => {
      const formatData = selectedFormats[videoId] || {
        max_resolution: 720,
        audio_only: false,
      };
      const requestBody = {
        url: videoUrl,
        sub_lang: selectedSubs[videoId] || null,
        max_resolution: formatData.max_resolution,
        audio_only: formatData.audio_only,
        audio_format: formatData.audio_format || "mp3",
      };
          ...
          const interval = setInterval(async () => {
            try {
              ...
              if (progData.status === "completed") {
                ...
                setDownloadProgress((prev) => ({
                  ...prev,
                  [videoId]: { status: "cancelled", progress: "0" },
                }));
                setTimeout(() => {
                  setDownloadProgress((prev) => ({ ...prev, [videoId]: null }));
                  setActiveTaskIds((prev) => ({ ...prev, [videoId]: null }));
  ...
  const downloadSingleVideo = async (videoUrl, videoId) => {
    setDownloadProgress((prev) => ({
      ...prev,
      [videoId]: { status: "starting", progress: "0", stream_type: "" },
    }));
    try {
      await downloadAndAwaitVideo(videoUrl, videoId);
    } catch (err) {
      if (err !== "cancelled") alert("Download failed: " + err);
    }
  };
  ...
  const getSelectAllText = () => {
    ...
    if (isPartial)
      return `Select All (${selectedVideos.size}/${videos.length})`;
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-white flex flex-col items-center p-4 md:p-8">
      ...
      <div className="flex items-center justify-center gap-2 md:gap-4 mb-1 md:mb-2 mt-4 md:mt-8">
        <svg
          className="w-10 h-10 md:w-14 md:h-14 drop-shadow-[0_0_10px_rgba(255,0,0,0.6)]"
          viewBox="0 0 100 100"
        ...
        >
          ...
          <path
            d="M40 15 L60 15 L60 55 L75 55 L50 85 L25 55 L40 55 Z"
            fill="white"
          />
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-6">
        <span className="bg-red-900/40 border border-red-600 px-3 py-1 rounded-full text-xs">
          🎬 Videos
        </span>

        <span className="bg-blue-900/40 border border-blue-600 px-3 py-1 rounded-full text-xs">
          📃 Playlists
        </span>

        <span className="bg-green-900/40 border border-green-600 px-3 py-1 rounded-full text-xs">
          🎵 MP3
        </span>

        <span className="bg-purple-900/40 border border-purple-600 px-3 py-1 rounded-full text-xs">
          📝 Subtitles
        </span>

        <span className="bg-yellow-900/40 border border-yellow-600 px-3 py-1 rounded-full text-xs">
          4K ✨
        </span>
      </div>
      <p className="text-[#AAAAAA] mb-8 md:mb-12 text-xs md:text-base font-medium tracking-wider uppercase">
        Personal Use • High Quality • Fast Proxy Support
      </p>

      {/* INPUT SECTION */}
      <div className="w-full max-w-3xl flex flex-col md:flex-row gap-3 md:gap-4 mb-8">
        <input
          type="text"
        ...
        <button
          ...
          className="bg-[#1a1a1a] border border-[#333] px-5 rounded-xl hover:bg-[#222]"
        >
          📋 Paste
        </button>
        <button
      ...
      {error && (
        <p className="text-red-400 mb-4 bg-red-900/30 px-6 py-3 rounded-lg border border-red-800/50 text-xs md:text-base backdrop-blur-sm">
          {error}
        </p>
      )}
      ...
      {videos.length === 0 && !loading && (
        <div className="mt-20 flex flex-col items-center text-[#444444]">
          <svg
            ...
            viewBox="0 0 24 24"
          >
            <path d="M10 16l5-4-5-4v8zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" />
          </svg>
        ...
        <div className="w-full max-w-4xl mb-6 bg-[#1a1a1a] p-5 md:p-6 rounded-2xl border border-[#282828] shadow-2xl transition-all duration-300">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="font-semibold text-[#F1F1F1] text-sm md:text-base">
                {getSelectAllText()}
              </span>
            </div>
            <button
              onClick={runPlaylistDownload}
              disabled={isDownloadingPlaylist || selectedVideos.size === 0}
          ...
          {isDownloadingPlaylist && (
            <div className="mt-2">
              <div className="flex justify-between text-xs md:text-sm text-white mb-1">
                ...
                <span className="text-[#AAAAAA] whitespace-nowrap">
                  {playlistQueue.current} / {playlistQueue.total}
                </span>
              </div>
              <div className="bg-[#121212] rounded-full h-3 w-full overflow-hidden">
                <div
                  className="bg-gradient-to-r from-red-500 to-red-700 h-full rounded-full transition-all duration-500 ease-out"
                  style={{
                    width: `${(playlistQueue.current / playlistQueue.total) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {videos.map((video) => (
          <div
            key={video.id}
            className={`bg-white/5 backdrop-blur-lg rounded-2xl overflow-hidden shadow-xl border transition-all duration-300 ease-out flex flex-col transform hover:scale-[1.02] ${selectedVideos.has(video.id) ? "border-red-600/70 shadow-[0_0_25px_rgba(255,0,0,0.15)]" : "border-[#282828] opacity-80 hover:opacity-100"}`}
          >
            <div className="relative group">
              {video.thumbnail && (
                <img
                  src={video.thumbnail}
                  alt={video.title}
                  className="w-full h-40 md:h-52 object-cover transition-opacity duration-300 group-hover:opacity-80"
                />
              )}
              {videos.length > 1 && (
                <input
            ...
            <div className="p-4 md:p-5 flex-1 flex flex-col">
              <h3 className="text-sm md:text-base font-semibold mb-3 line-clamp-2 text-[#F1F1F1]">
                {video.title}
              </h3>
              ...
              {downloadProgress[video.id] && (
                <div className="mb-4">
                    ...
                    <span>
                      {downloadProgress[video.id].status === "completed" ? (
                        ...
                        <span className="text-blue-300">
                          Downloading{" "}
                          {downloadProgress[video.id].stream_type || ""}...{" "}
                          {downloadProgress[video.id].speed || ""}
                        </span>
                      )}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 bg-[#121212] rounded-full h-2.5 md:h-3 w-full overflow-hidden border border-[#282828]">
                      <div
                        ...
                        style={{
                          width: `${downloadProgress[video.id].progress || 0}%`,
                        }}
                      ></div>
                    </div>
                  ...
                  {downloadProgress[video.id].eta &&
                    downloadProgress[video.id].status === "downloading" && (
                      <p className="text-[10px] md:text-xs text-[#555555] mt-1 text-right">
                        ETA: {downloadProgress[video.id].eta}
                      </p>
                    )}
                </div>
              )}
              ...
              {videoSettings[video.id]?.visible && (
                <div className="bg-[#121212] p-3 md:p-4 rounded-xl mb-3 border border-[#282828] shadow-inner transition-all duration-300">
                  {videoSettings[video.id].loading ? (
                    <p className="text-xs text-[#555555] animate-pulse">
                      Loading options...
                    </p>
                  ) : (
                    <>
                      <div className="mb-3">
                        <p className="text-[10px] md:text-xs text-[#555555] font-bold mb-1 uppercase tracking-wider">
                          Format
                        </p>
                        {videoSettings[video.id].formats.map((fmt) => (
                          <label
                            key={fmt.id}
                            className="flex items-center justify-between p-1.5 md:p-2 hover:bg-[#1a1a1a] rounded-lg cursor-pointer transition-colors"
                          >
                            <div className="flex items-center gap-2">
                              <input
                                type="radio"
                                ...
                                checked={
                                  selectedFormats[video.id]?.id === fmt.id
                                }
                                onChange={() =>
                            ...
                            <span className="text-[10px] md:text-xs text-[#555555] font-mono">
                              {fmt.size}
                            </span>
                          </label>
                        ))}
                      ...
                      {videoSettings[video.id].subs.length > 0 &&
                        !selectedFormats[video.id]?.audio_only && (
                          <div>
                            <p className="text-[10px] md:text-xs text-[#555555] font-bold mb-1 uppercase tracking-wider">
                              Subtitle (Embedded)
                            </p>
                            <select
                              className="w-full bg-[#1a1a1a] border border-[#282828] text-xs md:text-sm rounded-lg p-2 text-white focus:border-blue-500 focus:outline-none transition-colors"
                              value={selectedSubs[video.id] || ""}
                              onChange={(e) =>
                            ...
                            >
                              ...
                              {videoSettings[video.id].subs.map((sub) => (
              ...
              {videos.length <= 1 && (
                <button
                  onClick={() => downloadSingleVideo(video.url, video.id)}
                  disabled={
                    downloadProgress[video.id]?.status === "downloading" ||
                    downloadProgress[video.id]?.status === "merging" ||
                    downloadProgress[video.id]?.status === "starting"
                ...
                >
                  {downloadProgress[video.id]?.status === "completed" ? (
                    ...
                    <span>
                      ⬇ Download {selectedFormats[video.id]?.label || "720p"}
                    </span>
                  )}
                </button>
              )}
      ...
      <footer className="w-full max-w-4xl mt-16 pt-6 border-t border-[#282828] flex flex-col md:flex-row justify-between items-center text-[#555555] text-xs md:text-sm pb-8 px-4">
        <div className="flex items-center gap-2 mb-4 md:mb-0">
          ...
          <span className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-purple-500 text-base">
            Naga
          </span>
        </div>
        ...
        <div className="flex items-center gap-6">
          <a
            href="https://github.com/bnvreddy/youtube-downloader"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-white transition-colors flex items-center gap-2 group"
          >
            <svg
              className="w-5 h-5 group-hover:fill-white transition-colors"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
