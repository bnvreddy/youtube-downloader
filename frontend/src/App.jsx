...
function App() {
  ...
  const fetchVideoSettings = async (videoUrl, videoId) => {
    ...
    try {
      const fmtRes = await fetch('/api/list-formats', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: videoUrl }) });
      if (fmtRes.ok) { const fmtData = await fmtRes.json(); formats = fmtData.formats || []; }
    } catch (err) { console.error("Format fetch error:", err); }

    try {
      const subRes = await fetch('/api/list-subs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: videoUrl }) });
  ...
  const cancelDownload = async (videoId) => {
    ...
    if (task_id) {
      try { await fetch(`/api/cancel-download/${task_id}`, { method: 'POST' }); } 
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(data => {
        ...
        const interval = setInterval(async () => {
          try {
            const progRes = await fetch(`/api/progress/${task_id}`);
            ...
            if (progData.status === 'completed') {
              ...
              triggerDownload(`/api/get-file/${task_id}`);
              setTimeout(() => {
